...
export const serializeNegotiationMessage = (
  domain: string,
  hostname: string,
  ...
  let offset = 0;
  const buffer = Buffer.alloc(32 + domainBuffer.length + hostnameBuffer.length);
  ...
  negotiateFlags = typeof negotiateFlags === "number" ?
    negotiateFlags :
  ...
  const domainBufferPointerOffset = offset;
  offset += 8;

  const hostnameBufferPointerOffset = offset;
  offset += 8;

  serializeBufferPointer(domainBuffer.length, offset).copy(buffer, domainBufferPointerOffset);
  domainBuffer.copy(buffer, offset);
  offset += domainBuffer.length;

  serializeBufferPointer(hostnameBuffer.length, offset).copy(buffer, hostnameBufferPointerOffset);
...
export const serializeChallengeMessage = (
  ...
  if ((negotiateFlags & NegotiateFlag.TargetInfo) === 0) {
    ...
    targetInfoBuffer = attributeValue.serializePairs(targetInfo);
  }
  ...
  if (shouldSupplyVersion) {
    version = version instanceof Version ?
      version :
      Version.Server2003;

    version.serialize().copy(buffer, offset);
    offset += 8;
  }
...
export const parseChallengeMessage = (buffer: Buffer) => {
  ...
  const targetInfo = attributeValue.parsePairs(
    buffer.slice(
...
export const serializeAuthenticationMessage = (
  username: string,
  hostname: string,
  domain: string,
  serverChallenge: Buffer,
...
export const parseMessageType = (buffer: Buffer) => {
  return buffer.readUInt32LE(8) as MessageType;
};
...
export const matchExtendedSessionSecurityPasswordV2 = (password: string, serverChallenge: Buffer, lmResponse: Buffer, ntResponse: Buffer, domain: string, username: string) => {
...
export const createLmHash = (text: string) => {
...
export const createNtHash = (value: string) => {
  const buffer = Buffer.from(value, "ucs2");
  return createMd4HashBuffer(buffer);
};

export const createLmResponse = (password: string, serverChallenge: Buffer) => {
...
export const createNtResponse = (password: string, serverChallenge: Buffer) => {
...
export const createV2Hash = (domain: string, username: string, password: string) => {
...
export const isNtV2Response = (ntResponse: Buffer) => (
...
export const fixOddParity = (buffer: Buffer) => {
...
export const createDesKey = (key56: Buffer) => {
...
export const createResponse = (hash: Buffer, serverChallenge: Buffer) => {
